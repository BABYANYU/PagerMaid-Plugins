...
def format_entries(paths: Iterable[Path]) -> List[str]:
    return [
        ...
        f'{html.escape(DESCRIPTION_OVERRIDES[path.name] if path.name in DESCRIPTION_OVERRIDES else describe_plugin(path))}'
...
def split_messages(entries: List[str]) -> List[str]:
    if not entries:
        return ["<b>YanyuBot-插件列表</b>\n\n没有找到插件脚本。"]

    messages: List[str] = []
    header = "<b>YanyuBot-插件列表</b>"
    current = header
